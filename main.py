...
class WebCam:
    ...
    def __init__(self, camera_index: int = 0):
        """Method to initialise an instance of the WebCam class. It sets up the
        webcam device for capturing video and initializes various attribute variables
        used for monitoring and motion detection within the specified area.
        Arguments:
        - camera_index (int): The index of the camera device to be used for video capture.
                              The default value, 0, typically refers to the built-in
        ...
        # Initialize first_frame to None, to be set on first loop of video capture
        self.first_frame = None
        # Initialize an empty list to track changes in motion detection status
        self.status_list = []
        # Initialize entry_time to None, to be set and updated when motion is first detected
    ...
    def send_email(self, entry_time: datetime, in_camera: bool, attachment_filename: str):
        """Method to compose and send an email notification based on the motion detection
        status within the monitored area.
        Args:
        - entry_time (datetime): The timestamp when the motion was first detected.
        - in_camera (bool): A flag indicating whether the object is in the camera view or has left.
        - attachment_filename (str): The file path of the image captured during the detection, used as an email attachment.
        """
    ...
    def execute_concurrently_in_background(self, target_process, process_args: tuple = ()):
        """Execute a given process concurrently in a background thread
        Args:
        - target_process (function): The function to be executed in the background.
        - process_args (tuple, optional): Arguments to pass to the target_process function.
        """
    ...
    def monitor(self):
        """
        Continuously monitors the webcam feed to detect significant motion and
        manage alerts.
        This method captures frames from the webcam, processes them to detect
        motion in real-time, and takes action based on the presence or absence
        of significant motion. Actions include capturing images, sending alert
        emails, and managing internal state related to motion detection.
        """
        ...
        while True:
            ...
            # Convert the current frame to grayscale to simplify the image processing
            gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            # Apply Gaussian Blur to smooth the current frame, reducing detail and noise
            gray_frame_gau = cv2.GaussianBlur(gray_frame, (21, 21), 0)

            # Initialize the first frame to be used as a baseline for motion detection
            ...
            # Apply a threshold to the delta frame to raise the array value that is higher ...
            # ...than 60 to 255. The resulted frame should only consist of either 0 (black)...
            # ...or 255 (white)
            thresh_frame = cv2.threshold(delta_frame, 60, 255, cv2.THRESH_BINARY)[1]
            # Dilate the thresholded frame to fill in holes, making contours more detectable
            dil_frame = cv2.dilate(thresh_frame, None, iterations=2)
            # Find contours in the dilated frame which represent boundaries of moving objects
            contours, _ = cv2.findContours(dil_frame, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            # Iterate through each found contour
            for contour in contours:
                # Check the area of the contour to determine whether it's a target object
                if cv2.contourArea(contour) >= 5000:
                    # If so, compute the attributes of the bounding box of the contour, ...
                    # ...including (x, y) of the top left corner, and width and height of the ...
                    # ...bounding box
                    x, y, w, h = cv2.boundingRect(contour)
                    # Draw a rectangle on the current original frame based on the bounding box ...
                    # ...attributes
            ...
            if (status == 1) and (self.status_list[-1] == 0):
                # Record the current time as the entry time when motion is first detected
            ...
            if self.entry_time:
                ...
                # Calculate the duration in seconds since the motion was first detected
                duration_sec = (current_time - self.entry_time).seconds
                # Compare the duration against the predefined threshold to determine ...
                # ...if it is an extended presence
                if duration_sec >= self.DURATION_THRESHOLD:
                    # If so, check if an alert email has not already been sent for this presence
                    if not self.alert_email_sent:
                        # Generate a filename for saving an image of the detected object ...
                        # ...using the current timestamp
                        file_name = 'detected_object_{}.jpg'.format(current_time.strftime('%Y%m%d%H%M%S'))
                        # Save the current frame as a JPEG file
                        cv2.imwrite(file_name, frame)
                        # Execute the send_email method concurrently in the background to send a ...
                        # ...security alert email that notifies users of the extended presence
                        self.execute_concurrently_in_background(
                                target_process=self.send_email,
                                process_args=(self.entry_time, True, file_name)
                                )
                        # Set the alert-email-sent flag to True to prevent sending duplicate alert emails
